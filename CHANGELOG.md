...
### Fixed
- **Security**: Prevented predictable audit log and temporary file naming by appending UUIDs to generated filenames.
...
### Fixed
- **Performance**: Optimized cloud provider status checking in settings to run in parallel rather than sequentially. This reduces the loading time from the sum of all provider latencies to the maximum of a single provider latency.

## [1.0.5]

### Changed
- **Code Health**: Refactored `SystemState` application detection logic to use static constants and a helper method, replacing hardcoded strings for better maintainability.
